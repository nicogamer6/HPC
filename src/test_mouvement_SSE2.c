...
void test_routineFD_SSE_OMP(int seuil)
{
    ...
    for(i=1;i<=NBIMAGES;i++){
        ...
        //TODO convertir m en uint **
        uint_to_vuint(a, It, n1, n2, n3, n4);
        CHRONO(routine_FrameDifference_SSE2_OMP(I0,It,m,n1, n2,n3,n4,seuil),cycles);
        totalcy += cycles;
        vuint_to_uint(out, m, n1, n2, n3, n4);
    ...
    BENCH(printf("Cycles FD_SSE_OMP = "));
    BENCH(printf(format,totalcy));
    //printf("%ld",((nch+1)*(nrh+1)));
...
void test_routineSD_SSE_OMP()
{
    ...
    SigmaDelta_step0_SSE2_OMP (Mtm1, Vtm1, Itm1v, n1, n2, n3, n4);
    ...
    BENCH(printf("Cycles SD_SSE_OMP = "));
    BENCH(printf(format,totalcy));
...
{
    //vuint8 im =
    vuint8 M = init_vuint8_all(255,126, 255, 1, 0, 254, 24, 250,128, 0, 10, 1, 0, 254,128, 200);
    vuint8 It = init_vuint8_all(255,128, 255, 1, 0, 251, 24, 250,127, 0, 11, 1, 1, 255,127, 1);
    vuint8 Vtm1 = init_vuint8_all( 255, 128, 1, 0, 250, 254, 25, 0,128, 0, 10, 1, 1, 254, 127,200);
